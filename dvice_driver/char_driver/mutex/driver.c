/* This is a simple example of mutex. here we have taken 2 threads to show that mutex lock and unlock between them. if we use 3 threads then thread synchronisation will not happen without semaphore.
*/
...
int thread_function3(void *pv)
{
    while(!kthread_should_stop()) {
        mutex_lock(&etx_mutex);
        etx_global_variable++;
        printk(KERN_INFO "In EmbeTronicX Thread Function3 %lu\n", etx_global_variable);
        mutex_unlock(&etx_mutex);
        msleep(1000);
    }
    return 0;
