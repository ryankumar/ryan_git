...
void tasklet_fn(unsigned long arg)
{
	...
	//tasklet always run in interrupt context
	if(in_interrupt())
        printk(KERN_INFO "Running as an interrupt");
	else
        printk(KERN_INFO "Running as a process");
	printk(KERN_INFO "\n The state is %d",tasklet.state); // state =1 means TASKLET_STATE_SCHED and state=2 means TASKLET_STATE_RUN
	printk(KERN_INFO "\n The count is %d",tasklet.count); // count=0 means tasklet is enabled, nonzero means tasklet is disable
}
...
static int __init etx_driver_init(void)
{
	...
	/* Init the tasklet at Dynamic Method */
	// below six lines are used for dynamic tasklet only
/*
